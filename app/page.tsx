  ...
  return <main className={`game${population ? ' living' : ''}`}>
    ...
    {!!population && village && <section className={`village-panel village-compact glass${panel ? ' panel-open' : ''}`} aria-label="Village controls">
      ...
      {panel && <div className="village-body" id="village-detail-panel">
        ...
        {village.dock&&<button className="collect-offerings" disabled={village.dock.state!=='none'||!village.dock.canBuild} onClick={()=>{if(api.current?.buildBoat(village.dock!.id)){setVillage(api.current.status());setNotice('Followers will build a fishing boat at the dock.');}}}><ShipWheel />{village.dock.state==='none'?'Build fishing boat · 6 wood':village.dock.state==='building'?`Building boat · ${Math.round(village.dock.progress*100)}%`:village.dock.state==='at-sea'?`Fishing at a deep-water school · ${village.dock.trips} trips`:!village.dock.hasStore?`${village.dock.fish} fish waiting · Build a granary`:`Villagers unloading ${village.dock.fish} fish · ${village.dock.trips} trips`}</button>}
