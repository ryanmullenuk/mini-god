  ...
  return <main className={`game${population ? ' living' : ''}`}>
    ...
    {!!population && village && <section className={`village-panel village-compact glass${panel ? ' panel-open' : ''}`} aria-label="Village controls">
      ...
      {panel && <div className="village-body" id="village-detail-panel">
        ...
        <div className="god-powers icon-powers belief-powers" aria-label="Belief powers">{([
          {id:'rain',name:'Rain',Icon:CloudRain,cost:8},{id:'bloom',name:'Growth',Icon:Sprout,cost:12},
        ] as const).map(({id,name,Icon,cost})=><button key={id} aria-label={`${name} · ${cost} faith`} title={`${name} · ${hints[id]}`} aria-pressed={tool===id} className={tool===id?'active-guidance':''} disabled={paused||village.faith<cost} onClick={()=>chooseGuidance(id)}><Icon/><span>{name}</span></button>)}</div>
        </div><div hidden={panel!=='build'}>
        <div className="god-powers icon-powers" aria-label="Buildings">{([
          ...
          {id:'rally',name:'Gather',Icon:Flag,cost:0},
