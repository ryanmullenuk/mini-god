...
export default function Home() {
  ...
  function exportIsland() {
    ...
    anchor.href = url; anchor.download = `mini-god-day-${village?.day ?? 1}.json`; anchor.click();
  ...
  return <main className={`game${population ? ' living' : ''}`}>
    ...
    <div className="island-label"><span className="live-dot" />Day {village?.day ?? 1} · {paused ? 'Paused' : `${village?.timeOfDay ?? 'Dawn'}${village?.raining ? ' · Rain' : ''}`}</div>
    ...
    {!!population && village && <section className={`village-panel village-compact glass${panel ? ' panel-open' : ''}`} aria-label="Village controls">
      ...
      {panel && <div className="village-body" id="village-detail-panel">
        ...
        <div className="god-powers icon-powers" aria-label="Buildings and god powers">{([
          {id:'guide-home',name:'Hut',Icon:House,cost:6},{id:'guide-farm',name:'Farm',Icon:Wheat,cost:3},
    ...
    {(!ready||!started) && <div className="loading mini-god-splash"><SplashBeach/><div className="splash-content"><h2>MINI GOD</h2><p>A little world in your hands</p>{error?<div className="error-message" role="alert">{error}</div>:<button disabled={!ready} onClick={enterWorld}>{ready?'Enter your world':'Shaping your island…'}</button>}</div></div>}
    ...
    <Dialog open={help} onOpenChange={setHelp}><DialogContent className="help-content living-help"><DialogTitle>A little world, finding its way.</DialogTitle><DialogDescription>Shape the terrain. Your settlers decide how to live on it.</DialogDescription>
      ...
      <div className="save-actions"><button disabled={!ready} onClick={exportIsland}><Download />Export island</button><button disabled={!ready} onClick={() => file.current?.click()}><Upload />Import island</button><button disabled={!hasPrevious} onClick={() => { setNotice(api.current?.restorePrevious() ?? ''); if (api.current) setVillage(api.current.status()); }}><RotateCcw />Restore previous</button><button disabled={!ready} onClick={() => { setHelp(false); setReset(true); }}>New archipelago…</button></div>
    </DialogContent></Dialog>
    <Dialog open={reset} onOpenChange={setReset}><DialogContent className="help-content"><DialogTitle>Begin a new archipelago?</DialogTitle><DialogDescription>The main island is surrounded by seven smaller islands, each with its own trees, hills and beaches. Your current island will be kept as the previous island on this device. Export it first if you want a separate copy.</DialogDescription><div className="save-actions"><button onClick={exportIsland}><Download />Export current island</button><button onClick={() => setReset(false)}>Keep playing</button><button onClick={() => { setNotice(api.current?.resetWorld() ?? ''); setReset(false); setPaused(false); setTool('move'); setShapeOpen(false); if (api.current) setVillage(api.current.status()); api.current?.home(); }}>Begin new archipelago</button></div></DialogContent></Dialog>
  </main>;
}
