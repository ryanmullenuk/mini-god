...
export function createGame(host:HTMLDivElement,onReady:()=>void,onHistory:(n:number)=>void,onGuidance:(message:string)=>void=()=>{}):GameAPI{
 ...
 if(settings.tool==='move'&&e.type!=='pointercancel'&&e.button===0&&pointers.size===1&&Math.hypot(e.clientX-clickStart.x,e.clientY-clickStart.y)<6&&document.elementFromPoint(e.clientX,e.clientY)===canvas){
  makeRay(e.clientX,e.clientY);const picked=islanders.pick(ray);if(picked!==null){followingIslander=picked;const follower=simulation.state.settlers[picked];if(follower){target.set(follower.x,terrain.height(follower.x,follower.z)+.7,follower.z);view=host.clientWidth<600?6.5:8;pitch=.66;updateCamera();}}
 }
