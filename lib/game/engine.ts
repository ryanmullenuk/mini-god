...
import { IslandWeather } from './weather';
...
import { BUILD_LABEL, type GuidanceKind, type GuidancePreview, type SettlementStatus, type WorldState } from './world-state';
export type Tool='guide-torch'|'guide-bonfire'|'guide-dock'|'guide-granary'|'guide-storehouse'|'fishing'|'trap'|'guide-coop'|'guide-pigpen'|'settle'|'move'|'raise'|'lower'|'path'|'guide-home'|'guide-farm'|'rally'|'guide-temple'|'guide-slaughterhouse'|'rain'|'bloom';
const guideKind=(tool:Tool):GuidanceKind|null=>tool==='guide-torch'?'torch':tool==='guide-bonfire'?'bonfire':tool==='guide-dock'?'dock':tool==='guide-granary'?'granary':tool==='guide-storehouse'?'storehouse':tool==='fishing'?'fishing':tool==='trap'?'trap':tool==='guide-coop'?'coop':tool==='guide-pigpen'?'pigpen':tool==='settle'?'settle':tool==='guide-home'?'home':tool==='guide-farm'?'farm':tool==='guide-temple'?'temple':tool==='guide-slaughterhouse'?'slaughterhouse':tool==='rally'?'rally':tool==='rain'?'rain':tool==='bloom'?'bloom':null;
const sculptTool=(tool:Tool):tool is 'raise'|'lower'|'path'=>tool==='raise'||tool==='lower'||tool==='path';
export type GameSettings={tool:Tool;brush:number;paused:boolean;speed:1|3;showPlots:boolean;showInfluence:boolean;shadows:boolean};
export type GameAPI={selectedBuilding:()=>{id:number;name:string;moving:boolean}|null;beginMoveBuilding:(id:number)=>boolean;deleteBuilding:(id:number)=>boolean;rotateBuilding:(id:number)=>boolean;cameraStatus:()=>{heading:number;overhead:boolean};north:()=>void;topView:()=>void;upgradeHome:(id:number)=>boolean;buildBoat:(id:number)=>boolean;foodRole:(kind:'fish'|'keeper',id:number,worker:number|null)=>boolean;foodPriority:(id:number,value:'breed'|'food')=>boolean;trainHunting:(id:number)=>boolean;toggleHunting:(id:number)=>boolean;rearmTrap:(id:number)=>boolean;settings:GameSettings;zoom:(d:number)=>void;home:()=>void;focusVillage:()=>void;undo:()=>void;addIslanders:()=>number;inspectTerrain:(x:number,z:number)=>TerrainFacts|null;assessPlot:(kind:PlotKind,bounds:PlotBounds,context?:PlotContext)=>PlotAssessment;status:()=>SettlementStatus;placement:()=>GuidancePreview|null;cancelOrder:(id:number)=>boolean;dismissBeacon:()=>boolean;collectOfferings:()=>number;assignFarmer:(plotId:number,workerId:number|null)=>boolean;focusGuidance:(id:number)=>void;power:(kind:'rain'|'bloom')=>boolean;exportSave:()=>string;importSave:(raw:string)=>string;resetWorld:()=>string;restorePrevious:()=>string;hasPrevious:()=>boolean;saveStatus:()=>string;dispose:()=>void};
export function createGame(host:HTMLDivElement,onReady:()=>void,onHistory:(n:number)=>void,onGuidance:(message:string)=>void=()=>{}):GameAPI{
 const scene=new THREE.Scene();scene.background=new THREE.Color('#334b58');scene.fog=new THREE.FogExp2('#91a8ad',.0012);
 const renderer=new THREE.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(window.devicePixelRatio,2));renderer.setSize(host.clientWidth,host.clientHeight);renderer.shadowMap.enabled=true;renderer.shadowMap.autoUpdate=false;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.01;renderer.setClearColor('#334b58');host.appendChild(renderer.domElement);
 const tiltShift=new TiltShift();
 ...
 const weather=new IslandWeather();scene.add(weather.group);
 ...
 const settings:GameSettings={tool:'raise',brush:4,paused:false,speed:1,showPlots:false,showInfluence:false,shadows:true};
 ...
 const history:Float32Array[]=[];let stroke:SculptStroke|null=null,selectedBuildingId:number|null=null,movingBuildingId:number|null=null;
 ...
 function changeAt(p:THREE.Vector3,strength:number){if(!stroke)return;const waterRevision=terrain.waterRevision;if(terrain.applyStroke(stroke,p.x,p.z,settings.brush,strength)){if(terrain.waterRevision!==waterRevision)simulation.metadata.invalidate();const r=settings.brush+1;const b=strokeBounds??={minX:p.x-r,maxX:p.x+r,minZ:p.z-r,maxZ:p.z+r};b.minX=Math.min(b.minX,p.x-r);b.maxX=Math.max(b.maxX,p.x+r);b.minZ=Math.min(b.minZ,p.z-r);b.maxZ=Math.max(b.maxZ,p.z+r);dirty=true;strokeChanged=true;}}
 ...
 let sceneryPending=false,sceneryStep=0;
 const mesher=new TerrainMesher(terrain,()=>{sceneryPending=true;sceneryStep=0;renderer.shadowMap.needsUpdate=true;},()=>new GeometryWorker());
 /* One scenery system per frame: rebuilding wildlife, landscape and land animals together put their whole cost into the first frame after a stroke. A newer edit resets the sequence. */const sceneryTasks:(()=>void)[]=[()=>wildlife.terrainChanged(),()=>landscape.terrainChanged(),()=>landAnimals.terrainChanged()];function refreshScenery(){if(!sceneryPending||sculpting||mesher.busy)return;sceneryTasks[sceneryStep++]();if(sceneryStep>=sceneryTasks.length){sceneryStep=0;sceneryPending=false;}}
 function rebuild(){if(!dirty)return;mesher.request();dirty=false;}
 function finishStroke(){if(sculpting){rebuild();if(strokeChanged&&strokeSnapshot){simulation.terrainChanged(false,strokeBounds);villageView.terrainChanged();history.push(strokeSnapshot);if(history.length>35)history.shift();onHistory(history.length);}}sculpting=false;strokeBounds=undefined;strokeSnapshot=null;stroke=null;strokeChanged=false;lastStampPoint=null;touchPending=false;}
 ...
 if(settings.tool==='move'&&e.type!=='pointercancel'&&e.button===0&&pointers.size===1&&Math.hypot(e.clientX-clickStart.x,e.clientY-clickStart.y)<6&&document.elementFromPoint(e.clientX,e.clientY)===canvas){
  if(movingBuildingId!==null){const p=pick(e.clientX,e.clientY),result=p?simulation.relocatePlot(movingBuildingId,{x:Math.round(p.x*2)/2,z:Math.round(p.z*2)/2}):null;if(result){onGuidance(result.message);if(result.allowed){movingBuildingId=null;saveNow();landscape.terrainChanged();}}}
  else{makeRay(e.clientX,e.clientY);const picked=islanders.pick(ray);if(picked!==null){selectedBuildingId=null;villageView.selectPlot(null);followingIslander=picked;const follower=simulation.state.settlers[picked];if(follower){target.set(follower.x,terrain.height(follower.x,follower.z)+.7,follower.z);view=host.clientWidth<600?6.5:8;pitch=.66;updateCamera();}}
  else{selectedBuildingId=villageView.pickPlot(ray);villageView.selectPlot(selectedBuildingId);}}
 }
 ...
 function wheel(e:WheelEvent){e.preventDefault();followingIslander=null;view=THREE.MathUtils.clamp(view*Math.exp(e.deltaY*.001),3.8,150);updateCamera();}
 ...
 simulation.advance(dt*settings.speed,stopped);islanders.sync(simulation.state.settlers,dt*settings.speed,stopped);if(followingIslander!==null){const follower=simulation.state.settlers[followingIslander];if(follower){target.x=THREE.MathUtils.damp(target.x,follower.x,7,dt);target.y=THREE.MathUtils.damp(target.y,terrain.height(follower.x,follower.z)+.7,7,dt);target.z=THREE.MathUtils.damp(target.z,follower.z,7,dt);updateCamera();}else followingIslander=null;}villageView.update(simulation,settings.showPlots,settings.showInfluence||settings.tool==='rain'||settings.tool==='bloom');
 if(now-lastSave>15000&&!sculpting&&!mesher.busy){saveNow();lastSave=now;}
 const light=daylight.update(simulation.state.time);if(renderer.shadowMap.enabled!==settings.shadows){renderer.shadowMap.enabled=settings.shadows;renderer.shadowMap.needsUpdate=settings.shadows;}if(settings.shadows&&Math.floor(elapsed*8)!==Math.floor((elapsed-dt)*8))renderer.shadowMap.needsUpdate=true;ocean.setLighting(light.direction,light.colour,light.strength,light.tint,scene.background as THREE.Color);landscape.setLighting(light.tint);
 if(scene.fog instanceof THREE.FogExp2)scene.fog.density=THREE.MathUtils.lerp(.0007,.0027,THREE.MathUtils.smoothstep(view,30,120));
 foodView.update(simulation.state);landscape.update(simulation.state,elapsed);landAnimals.sync(simulation.state.foodSystem.animals,dt*settings.speed,stopped);landAnimals.update(dt*settings.speed,stopped,false);weather.update(dt,simulation.raining,stopped);wildlife.managedFishing=simulation.state.foodSystem.fishing;wildlife.setCloudsVisible(view>38);ocean.update(elapsed,camera);if(skyPointer){makeRay(skyPointer.x,skyPointer.y);wildlife.setCursorRay(ray.ray,Math.max(1.5,view*.07));}wildlife.update(dt,settings.paused);marine.update(dt,settings.paused);tiltShift.render(renderer,scene,camera,view);
 ...
 return {settings,
 selectedBuilding(){if(selectedBuildingId===null)return null;const p=simulation.state.plots.find(p=>p.id===selectedBuildingId&&p.valid&&p.stage==='complete');return p?{id:p.id,name:BUILD_LABEL[p.kind],moving:movingBuildingId===p.id}:null;},
 beginMoveBuilding(id){if(!simulation.state.plots.some(p=>p.id===id&&p.valid&&p.stage==='complete'))return false;movingBuildingId=id;return true;},
 deleteBuilding(id){const ok=simulation.deletePlot(id);if(ok){selectedBuildingId=null;movingBuildingId=null;villageView.selectPlot(null);landscape.terrainChanged();saveNow();}return ok;},
 rotateBuilding(id){const p=simulation.state.plots.find(p=>p.id===id&&p.valid&&p.stage==='complete');if(!p||p.kind==='dock')return false;p.rotation=THREE.MathUtils.euclideanModulo((p.rotation??0)+Math.PI/12,Math.PI*2);saveNow();return true;},
 ...
 buildBoat(id){const ok=simulation.buildBoat(id);if(ok)saveNow();return ok;},
 ...
 focusVillage(){const p=simulation.state.camp;if(p){followingIslander=null;target.set(p.x,terrain.height(p.x,p.z),p.z);view=host.clientWidth<600?13:18;updateCamera();}},inspectTerrain:(x,z)=>simulation.metadata.inspect(x,z),assessPlot:(kind,bounds,context)=>simulation.metadata.assessPlot(kind,bounds,context),cameraStatus:()=>({heading:yaw*180/Math.PI,overhead:pitch>1.5}),north(){yaw=0;updateCamera();},topView(){followingIslander=null;const overhead=pitch>1.5;view=host.clientWidth<600?116:98;yaw=0;pitch=overhead?.82:Math.PI/2-.001;target.set(0,1.5,0);updateCamera();},zoom(d){view=THREE.MathUtils.clamp(view+d,3.8,150);updateCamera();},home(){followingIslander=null;view=host.clientWidth<600?116:98;yaw=.30;pitch=.82;target.set(0,1.5,0);updateCamera();},addIslanders(){const n=simulation.add(2);saveNow();return n;},undo(){finishStroke();mesher.cancel();const saved=history.pop();if(saved){terrain.values.set(saved);terrain.rebuild();mesher.syncBaseline();simulation.terrainChanged();villageView.terrainChanged();wildlife.terrainChanged();landscape.terrainChanged();landAnimals.terrainChanged();onHistory(history.length);}},dispose(){finishStroke();mesher.dispose();saveNow();window.removeEventListener('pagehide',onPageHide);cancelAnimationFrame(frame);observer.disconnect();window.removeEventListener('blur',cancel);canvas.removeEventListener('pointerdown',pointerDown);canvas.removeEventListener('pointermove',pointerMove);canvas.removeEventListener('pointerup',pointerUp);canvas.removeEventListener('pointercancel',pointerUp);canvas.removeEventListener('pointerleave',leave);canvas.removeEventListener('wheel',wheel);canvas.removeEventListener('contextmenu',context);villageView.dispose();guidanceCursor.dispose();simulation.metadata.dispose();terrain.dispose();islanders.dispose();wildlife.dispose();marine.dispose();foodView.dispose();landscape.dispose();landAnimals.dispose();weather.dispose();ocean.dispose();ring.geometry.dispose();disk.geometry.dispose();ringMaterial.dispose();fillMaterial.dispose();tiltShift.dispose();renderer.dispose();host.replaceChildren();}};
}
