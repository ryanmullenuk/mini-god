...
export function createGame(host:HTMLDivElement,onReady:()=>void,onHistory:(n:number)=>void,onGuidance:(message:string)=>void=()=>{}):GameAPI{
 ...
 const history:Float32Array[]=[];let stroke:SculptStroke|null=null,selectedBuildingId:number|null=null;
 ...
 return {settings,
 selectedBuilding(){if(selectedBuildingId===null)return null;const p=simulation.state.plots.find(p=>p.id===selectedBuildingId&&p.valid&&p.stage==='complete');return p?{id:p.id,name:BUILD_LABEL[p.kind]}:null;},
