...
export type FishingBoat = {state:'building'|'at-sea'|'docked';progress:number;returnAt:number;departAt:number;arriveAt?:number;fishUntil?:number;trips:number;fish:number;targetX?:number;targetZ?:number;launchX?:number;launchZ?:number;schoolId?:number};
