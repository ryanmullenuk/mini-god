...
test('dawn-to-night lighting follows saved simulation time and wraps smoothly into the next day',()=>{
 ...
 daylight.update(195);assert.ok(sun.intensity<daytime);assert.ok(ambient.intensity>=.6,'Night retains enough ambient light to play');assert.equal(daylight.strength,0,'Sun glitter disappears at night');assert.ok(daylight.tint.r<dayTint.r);assert.ok(daylight.direction.distanceTo(morningDirection)>.3);
